...
$fields = 's_m.SO_LINE,s_m.CREATE_DATE,NUMBER_NO,REQ,PD,s_m.INTERNAL_ITEM,s_i.RBO,s_m.ITEM,ITEM_DES,s_m.QTY,MATERIAL_CODE,MATERIAL_DES,EA_SHT,YD,MT,MATERIAL_QTY,LENGTH,WIDTH,INK_CODE,INK_DES,INK_QTY,SO_UPS,FORM_TYPE';
// to do process so kho if(type_worst_vertical = 100-SB1) 10,5
$month_1 = date('ym', strtotime("-1 month")).'-';
$month_2 = date('ym').'-';
$month_3 = date('ym', strtotime("+1 month")).'-';
$query = "SELECT $fields FROM save_material as s_m JOIN save_item as s_i ON s_i.NUMBER_NO = s_m.ID_SAVE_ITEM where (ID_SAVE_ITEM like '%$month_1%' OR ID_SAVE_ITEM like '%$month_2%' OR ID_SAVE_ITEM like '%$month_3%') order by s_m.ID asc";
//echo $query;      die;  
