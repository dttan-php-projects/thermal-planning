<?php
    ...
    {
        $result = 'loading...';
        $data = MiQuery("SELECT `STATUS`, `CREATEDDATE` FROM autoload_log ORDER BY ID DESC;", _conn('au_avery') );
        if (!empty($data) ) {
            $data = $data[0];
            $status = $data['STATUS'];
            ...
            if ($status == 'OK' ) {
                ...
                $dataOK = MiQuery("SELECT `STATUS`, `CREATEDDATE` FROM autoload_log WHERE `STATUS`='OK' ORDER BY ID DESC;", _conn('au_avery') );
                $created_date_OK = '';
                if (!empty($dataOK) ) {
                    $dataOK = $dataOK[0];
	...
	// check role
	$rolesUpdateDB = ['phung.le','hang.nguyenthu','tri.pham','vien.trinh','yen.thai','thitram.nguyen','tuan.vi','trinh.truong', 'tan.doan', 'tan.doan1','hoang.dang', 'giau.duong', 'son.dang', 'keva.do', 'duy.dang', 'jimmy.dang', 'chanh.ht.nguyen'];
	$updateDB = 0;
...
?>
    ...
    function initMenu(){
        MainMenu = new dhtmlXMenuObject({
				...
				json: "./Module/FileHome/Menu.json",
				top_text: HeaderTile
        });
    }	
		...
		*/
        ...
        ToolbarMain.attachEvent("onClick", function(name)
        {
            ...
            else if(name == "view_no")
            {
                ...
                saveDatabase(false);
				// Nếu nhấn save rồi thì disable item save đi
				ToolbarMain.disableItem("saveNo");
            ...
            }else if(name == "view_oracle")
            {
				...
				// // if (print_type.toLowerCase() == 'paxar' ) {
				// // 	var url_export = RootDataPath+'export_form_type.php?print_type='+print_type+'&from_date_value='+from_date_value+'&to_date_value='+to_date_value;
			...
			{
				var url_oracle_download =  RootPath + 'upload_oracle_download.php';
				window.open(
    ...
    function loadListUA(){		
        ...
        if (!dhxWinsUA.isWindow("windowViewUA")){
			...
			UAGrid.setImagePath("./dhtmlx/skins/skyblue/imgs/");	
    ...
    function saveDatabase(print){		
		if(print_type=='paxar'){			
			...
			if(checked_SOLINE.length>0){
				...
				QTY = QTY.trim();
				QTY_CHECK = QTY;
				//QTY_CHECK_2 = Number(QTY);
				//console.log('qty_check: '+QTY_CHECK);
				...
				var test_dobule_check_1 = JSON.stringify(jsonObjects);
				// console.log(test_dobule_check_1); 
    ...
    function initNO(){			
		...
		if(print_type=='paxar'){
			if(idCheck){	
				...
				console.log('sample_check: '+sample_check);
				...
				LayoutMain.attachEvent("onContentLoaded", function(id){					
					...
					$("#frm_remark_3").val(SoGrid.cellByIndex(0,36).getValue() + " " + SoGrid.cellByIndex(0,28).getValue());		
					$('#frm_sample').change(function() {
						...
						if(order_type==0){ // don mau
							...
							$("#frm_remark_3").val(SoGrid.cellByIndex(0,28).getValue());		
	...
	function initGridPaxarToSave(){
		if(SoGrid.cellByIndex(0,37).getValue()=='1'){ // color by size
			...
			gridPaxarSave.setImagePath("./dhtmlx/skins/skyblue/imgs/");
	...
	function initGridTrimToSave(){
		...
		gridTrimSave.setImagePath("./dhtmlx/skins/skyblue/imgs/");
	...
	function initGridSipsToSave(){
		...
		gridSipsSave.setImagePath("./dhtmlx/skins/skyblue/imgs/");
    ...
    function initSoGrid(){
		...
		if(print_type == 'paxar'){
			...
			SoGrid.setColumnIds(",SO,LINE,QTY,ITEM,INTERNAL ITEM,RBO,PD,REQ,ORDER,ITEM DESCRIPTION,MATERIAL CODE,MATERIAL DESCRIPTION,EA-SHT,YD,MT,LENGTH,WIDTH,INK CODE,INK DESCRIPTION,INK QTY,UNIT,UPS,SHIP_TO,CS,ITEM,REMARK 1,REMARK 2,REMARK 3,REMARK 4,REMARK 5,MULTIPLE,DATA,SAMPLE,SO UPS,ORDER TYPE NAM,PACKING INSTR,COLOR BY SIZE,BILL_TO_CUSTOMER");         //sets the columns' ids
			SoGrid.setInitWidths("30,65,45,50,105,110,195,90,80,90,180,130,280,120,50,50,70,70,75,130,120,70,70,325,145,145,145,145,145,145,145,70,145,70,80,150,150,150,150");   //sets the initial widths of columns
			SoGrid.setColAlign("left,left,left,left,left,left,left,left,left,left,left,left,left,left,left,left,left,left,left,left,left,left,left,left,left,left,left,left,left,left,left,left,left,left,left,left,left,left,left");     //sets the alignment of columns
			SoGrid.setColTypes("ch,ed,ed,ed,ed,ed,ed,ed,ed,ed,ed,ed,ed,ed,ed,ed,ed,ed,ed,ed,ed,ed,ed,ed,ed,ed,ed,ed,ed,ed,ed,ed,ed,ed,ed,ed,ed,ed,ed");    //sets the types of columns
			SoGrid.setColSorting("na,na,na,na,na,na,na,na,na,na,na,na,na,na,na,na,na,na,na,na,na,na,na,na,na,na,na,na,na,na,na,na,na,na,na,na,na,na,na"); 
		}else if(print_type == 'trim'){
    ...
    function initMaterialGrid(){
        ...
        MaterialGrid.setImagePath("./dhtmlx/skins/skyblue/imgs/");
    ...
    function filterSO(){
        ...
        var keycode = (event.keyCode ? event.keyCode : event.which);
            if(keycode == '13'){ // enter on input text				
				...
				console.log('SOLINE: '+text);
                //save query string in global variable (see step 5 for details)				
				loadGridSO(text);
                var index = 0;		
				for (var i=0; i<SoGrid.getRowsNum(); i++){						
					if(print_type=='paxar'){
						// // @tandoan: 20200328. Check SOLine đã làm lệnh hiện thị thông báo cho người dùng
						// var order_number_check = SoGrid.cellByIndex(index,1).getValue();
						// var line_number_check = SoGrid.cellByIndex(index,2).getValue();
						// var so_line_double_check = order_number_check + line_number_check;
    ...
    $(document).ready(function(){		
        ...
        if (!VNRISIntranet ) {
            ...
            if (!pr || pr.indexOf('@') !== -1 ) {
                ...
                // setCookie('VNRISIntranet', pr, 30 );
                var VNRISIntranet = '<?php echo getUser(); ?>';
                var pr_s = '<?php echo planning_user_statistics($email, "Thermal_Planning_PAXAR"); ?>';
