<?php
	...
	// check role
	$rolesUpdateDB = ['phung.le', 'hang.nguyenthu', 'tri.pham', 'vien.trinh', 'yen.thai', 'thitram.nguyen', 'tuan.vi', 'trinh.truong', 'tan.doan', 'jimmy.dang'];
	$updateDB = 0;
...
?>
	...
	<head>
		...
		<script>
			...
			var MainMenu;
			var ToolbarMain;
			var RootPath = '<?php echo str_replace('INDEXT.PHP', '', strtoupper(str_replace('index__.php', '', $_SERVER['REQUEST_URI']))); ?>';
			...
			function initToolbar() {
				...
				*/
				...
				var opts_data = [
					['auto_mail', 'obj', 'AUTOMAIL'],
					['oracle_download', 'obj', 'ORACLE DOWNLOAD']
				];
			...
			function viewNO_SO() {
				...
				if (!dhxWins.isWindow("windowViewNo")) {
					...
					var from_date_value = ToolbarMain.getValue("from_date");
					var to_date_value = ToolbarMain.getValue("to_date");
					var so_search_value = ToolbarMain.getValue("so");
					//viewNOGrid.load(RootDataPath+'view_no.php');

					viewNOGrid.load(RootDataPath + 'view_no.php?so_search_value=' + so_search_value + '&from_date_value=' + from_date_value + '&to_date_value=' + to_date_value, function() {});
			...
			//@tandoan: Thay doi ngay 20191022. @Tram, Quang.Phan yeu cau thay đôi. Dựa vào ink_code
			var gridTrimSave;
				...
				if (SoGrid.getRowsNum() > 0) {
					...
					for (var index = 0; index < SoGrid.getRowsNum(); index++) {
						if (SoGrid.cellByIndex(index, 0).getValue() == 1) {
							...
							//@tandoan: check nếu ink_code = ink_code2: nhân đôi mực.
							//@Tram, Quang.Phan yeu cau. Update: 20191022
							if (MULTIPLE != "0") {
								var MULTIPLE_DATA_CHECK = SoGrid.cellByIndex(index, 23).getValue();
								if (MULTIPLE_DATA_CHECK) {
									MULTIPLE_DATA_CHECK = JSON.parse(MULTIPLE_DATA_CHECK);
									var INK_CODE_TMP2 = MULTIPLE_DATA_CHECK.INK_CODE;
									if (INK_CODE == INK_CODE_TMP2) {
										INK_QTY = INK_QTY * 2; //nhan doi muc
									}
								}
							...
							if (MULTIPLE != "0") {
								...
								if (MULTIPLE_DATA) {
									INK_CODE = MULTIPLE_DATA.INK_CODE;
									var INK_CODE_TMP = SoGrid.cellByIndex(index, 12).getValue();
								}

								if (INK_CODE_TMP == INK_CODE) {
									continue;
								}
			...
			$(document).ready(function() {
				...
				if (!VNRISIntranet ) {
					...
					if (!pr || pr.indexOf('@') !== -1 ) {
						...
						// setCookie('VNRISIntranet', pr, 30 );
						var VNRISIntranet = '<?php echo getUser(); ?>';
						var pr_s = '<?php echo planning_user_statistics($email, "Thermal_Planning_SIP"); ?>';
				...
				if (check_gg) location.href = './indexT.php';
